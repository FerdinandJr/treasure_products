...
sudo mysql -u root -p -e "ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'root@000!'; FLUSH PRIVILEGES;" || true
sudo mysql -u root --password=root@000! -e "CREATE DATABASE IF NOT EXISTS my_store;"

# Import database
echo "Importing database..."
sudo mysql -u root --password=root@000! my_store < /var/www/html/php_mysql_nginx_docker_treasure-products/my_store.sql
